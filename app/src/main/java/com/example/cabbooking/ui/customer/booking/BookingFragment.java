...
//        transaction.replace(R.id.booking_info, ratingFragment).commit();
    ...
    private void createNewBookingInDB() {
        ...
        data.put(Constants.FSBooking.declinedDrivers, new ArrayList<String>()); // Initialize empty declined drivers list
        data.put(Constants.FSBooking.rideOTP, null); // OTP will be generated when driver arrives
        data.put(Constants.FSBooking.otpVerified, false); // Initially not verified
     ...
     */
    private void sendDataToPopupDriverArrivalViewModel(Booking booking) {
        ...
        popupDriverArrivalViewModel.setBooking(booking);
    }
            ...
            public void onEvent(@Nullable DocumentSnapshot value, @Nullable FirebaseFirestoreException error) {
                ...
                if (value != null && value.exists()) {
                    ...
                    if (booking.getArrived()) {
                        sendDataToPopupDriverArrivalViewModel(booking);
    ...
    /**
     * A Class to call Google Directions API with callback using ExecutorService
     */
    private void fetchRouteData(String url) {
     ...
     */
    /**
     * Parse route data using ExecutorService
     */
    private void parseRouteData(String jsonData) {
        ...
        executor.execute(() -> {
            ...
            handler.post(() -> {
                ...
                drawRoute(finalRoutes); //Draw new route
            });
        });
    }
