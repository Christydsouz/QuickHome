package com.example.cabbooking.ui.driver.home;
...
import com.example.cabbooking.ui.driver.alert.DriverAlertFragment;
import com.example.cabbooking.ui.driver.alert.DriverAlertViewModel;
import com.example.cabbooking.ui.driver.driver_info.DriverInfoFragment;
import com.example.cabbooking.ui.driver.driver_info.DriverInfoViewModel;
...
//        updateCurrentDriverLocationOnDB(newLatLng);
    ...
    private void handleDeclineBooking() {
        if (currentBookingDocRef != null && currentUserObject != null) {
            // Add current driver to declined drivers list
            currentBookingDocRef.get().addOnSuccessListener(documentSnapshot -> {
                if (documentSnapshot.exists()) {
                    Booking booking = documentSnapshot.toObject(Booking.class);
                    if (booking != null) {
                        List<String> declinedDrivers = booking.getDeclinedDrivers();
                        if (declinedDrivers == null) {
                            declinedDrivers = new ArrayList<>();
                        }
                        
                        // Add current driver to declined list if not already present
                        if (!declinedDrivers.contains(currentUserObject.getDocId())) {
                            declinedDrivers.add(currentUserObject.getDocId());
                            
                            // Update the booking in Firestore
                            currentBookingDocRef.update(Constants.FSBooking.declinedDrivers, declinedDrivers)
                                .addOnSuccessListener(aVoid -> {
                                    // Successfully updated declined drivers list
                                    resetBookingFlow();
                                })
                                .addOnFailureListener(e -> {
                                    // Failed to update, just reset locally
                                    resetBookingFlow();
                                });
                        } else {
                            // Driver already in declined list, just reset
                            resetBookingFlow();
                        }
                    } else {
                        resetBookingFlow();
                ...
                // Failed to get booking, just reset locally
                resetBookingFlow();
            });
        } else {
            resetBookingFlow();
                    ...
                    public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException error) {
                        ...
                        for (QueryDocumentSnapshot doc : value) {
                            ...
                            if (booking.getAvailable() && booking.getTransportationType().equals(currentUserObject.getTransportationType())) {
                                // Check if current driver has already declined this booking
                                if (booking.getDeclinedDrivers() != null && 
                                    booking.getDeclinedDrivers().contains(currentUserObject.getDocId())) {
                                    continue; // Skip this booking, driver already declined
                                }
        ...
        driverHomeViewModel.getAcceptBookingBtnPressed().observe(getViewLifecycleOwner(), new Observer<Boolean>() {
            ...
            public void onChanged(Boolean aBoolean) {
                ...
                if (aBoolean) {
                    ...
                    handleDeclineBooking(); // Use the new decline handler
                }
                driverHomeViewModel.setAcceptBookingBtnPressed(null);
            }
        });
