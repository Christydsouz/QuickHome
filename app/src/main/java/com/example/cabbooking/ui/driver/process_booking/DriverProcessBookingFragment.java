...
import com.example.cabbooking.ui.driver.otp.OTPVerificationFragment;
import com.example.cabbooking.ui.driver.otp.OTPVerificationViewModel;
...
public class DriverProcessBookingFragment extends Fragment implements OnMapReadyCallback {
    ...
    /**
     * Update arrival status on DB and generate OTP
     */
    private void updateArrivalStatusOnDB(){
        ...
        currentBookingDocRef.update(
                Constants.FSBooking.arrived, true,
                Constants.FSBooking.rideOTP, otp,
                Constants.FSBooking.otpVerified, false
        ).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                // Update local booking object with the generated OTP
                currentBooking.setRideOTP(otp);
                currentBooking.setArrived(true);
                currentBooking.setOtpVerified(false);
            ...
            // Handle failure case
            if (getActivity() != null) {
                Toast.makeText(getActivity(), "Failed to update arrival status. Please try again.", Toast.LENGTH_SHORT).show();
            }
        });
    }

    /**
     * Show OTP verification dialog for driver to verify customer OTP
     */
    private void showOTPVerificationDialog() {
        // First refresh the booking data from Firestore to ensure we have the latest OTP
        currentBookingDocRef.get().addOnSuccessListener(documentSnapshot -> {
            if (documentSnapshot.exists()) {
                Booking refreshedBooking = documentSnapshot.toObject(Booking.class);
                if (refreshedBooking != null) {
                    ...
                    // Pass current booking data to OTP verification
                    OTPVerificationViewModel otpViewModel = new ViewModelProvider(requireActivity()).get(OTPVerificationViewModel.class);
                    otpViewModel.setCurrentBooking(currentBooking);
                    otpViewModel.setCurrentBookingDocRef(currentBookingDocRef);
                    
                    otpVerificationFragment.show(fm, "otp_verification_dialog");
                }
            }
        }).addOnFailureListener(e -> {
            if (getActivity() != null) {
                Toast.makeText(getActivity(), "Failed to load booking data. Please try again.", Toast.LENGTH_SHORT).show();
            }
        });
    }
...
//                addEventListenerForPickUpButton();
        ...
        //OTP verified
        mViewModel.getOtpVerified().observe(getViewLifecycleOwner(), new Observer<Boolean>() {
            @Override
            public void onChanged(Boolean aBoolean) {
                if (aBoolean == null) return;
                if (aBoolean) {
                    // OTP verified successfully, proceed to dropoff state
                    setViewInDropOffState();
                }
                mViewModel.setOtpVerified(null); // Clear the value
            }
        });
    }
...
//        setUpCluster(); //Set up cluster on Google Map
    ...
    private void drawRoute(List<List<HashMap<String, String>>> result)
    {
        ...
        for (int i = 0; i < result.size(); i++) {
            ...
            // Check if route is null or empty
            ...
            // Only add polyline if we have valid points
            if (!points.isEmpty()) {
